...
// Tambahkan tombol Game Menggambar di menu
function tambahTombolMenggambar() {
    const menu = document.getElementById('menu');
    if (!menu) return;
    
    // Cek apakah tombol sudah ada
    if (document.getElementById('btnMenggambar')) return;
    
    const btn = document.createElement('button');
    btn.id = 'btnMenggambar';
    ...
    btn.innerHTML = `
        ...
        <div class="kategori-nama">Game Menggambar</div>
        <div class="kategori-level">Hubungkan Titik!</div>
    `;
    
    // Tambahkan setelah kategori grid
    const grid = document.querySelector('.kategori-grid');
    if (grid) {
        grid.appendChild(btn);
    }
}

// Panggil saat menu ditampilkan
const originalTampilkanMenu = tampilkanMenu;
tampilkanMenu = function() {
    originalTampilkanMenu();
    setTimeout(tambahTombolMenggambar, 100);
};